<!--
  ...
  执行顺序：
    ...
    Orchestrator   : 合并 §5 改动文件清单 + §6 技术约束 + §7 不包含 → 写入 "specs/active/43-feature-account-registration-login-3-design.md"

  Subagent 间通信方式：临时文件（无需实验性 Agent Team 功能）
    "specs/active/43-feature-account-registration-login-3-design.md".architect.md   — architect 写入，其他 subagent 只读
    "specs/active/43-feature-account-registration-login-3-design.md".backend.md     — backend-developer 写入
    "specs/active/43-feature-account-registration-login-3-design.md".frontend.md    — frontend-developer 写入
  ===================================================
-->
---
...
1. 读取每个 spec 文件：`specs/active/43-feature-account-registration-login-3.md`
...
```bash
ls ""specs/active/43-feature-account-registration-login-3-design.md".architect.md" 2>/dev/null && echo "Phase1 OK" || echo "Phase1 WARN: architect.md not found"
```
...
## 第二步 — 生成内容并写入临时文件
将以下内容写入 "specs/active/43-feature-account-registration-login-3-design.md".architect.md（使用 Write 工具）：
...
完成后输出一行：WRITTEN: "specs/active/43-feature-account-registration-login-3-design.md".architect.md
```
...
## 第一步 — 读取上下文（只读，不写代码）
1. 读取 "specs/active/43-feature-account-registration-login-3-design.md".architect.md — architect 已定义的数据模型（必须基于此设计 API）
2. 读取每个 spec 文件："specs/active/43-feature-account-registration-login-3.md"

## 第二步 — 生成内容并写入临时文件
将以下内容写入 "specs/active/43-feature-account-registration-login-3-design.md".backend.md（使用 Write 工具）：
...
## 严禁事项
- 禁止向用户提问或等待确认 — 全程自主运行，遇到歧义以 spec 为准
- 禁止修改 "specs/active/43-feature-account-registration-login-3-design.md".architect.md（只读）

完成后输出一行：WRITTEN: "specs/active/43-feature-account-registration-login-3-design.md".backend.md
```
...
## 第一步 — 读取上下文（只读，不写代码）
1. 读取 "specs/active/43-feature-account-registration-login-3-design.md".architect.md — architect 已定义的数据模型
2. 读取每个 spec 文件："specs/active/43-feature-account-registration-login-3.md"

## 第二步 — 生成内容并写入临时文件
将以下内容写入 "specs/active/43-feature-account-registration-login-3-design.md".frontend.md（使用 Write 工具）：
...
完成后输出一行：WRITTEN: "specs/active/43-feature-account-registration-login-3-design.md".frontend.md
```
...
- `"specs/active/43-feature-account-registration-login-3-design.md".architect.md`（含 §1 §2）
- `"specs/active/43-feature-account-registration-login-3-design.md".backend.md`（含 §3）
- `"specs/active/43-feature-account-registration-login-3-design.md".frontend.md`（含 §4）
...
使用 Write 工具将完整文档写入 `"specs/active/43-feature-account-registration-login-3-design.md"`。
...
1. 使用 Read 工具读取 `"specs/active/43-feature-account-registration-login-3-design.md"`，确认包含以下所有章节标题：
...
已写入文件：
WRITTEN: "specs/active/43-feature-account-registration-login-3-design.md"
```
