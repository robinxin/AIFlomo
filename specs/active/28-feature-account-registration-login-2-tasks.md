...
- [x] T013 创建 use-auth Hook，封装 register 方法（调用 API + dispatch action） `apps/mobile/hooks/use-auth.js`
- [x] T014 [P] 创建 TextInput 通用组件（支持邮箱、密码、错误提示） `apps/mobile/components/TextInput.jsx`
- [x] T015 [P] 创建 Button 通用组件（支持加载状态、禁用状态） `apps/mobile/components/Button.jsx`
- [x] T016 [P] 创建 PolicyCheckbox 隐私协议复选框组件 `apps/mobile/components/PolicyCheckbox.jsx`
- [x] T017 创建认证路由组布局（Stack 导航，无 header） `apps/mobile/app/(auth)/_layout.jsx`
- [x] T018 实现注册页面（表单、前端校验、提交逻辑、跳转） `apps/mobile/app/(auth)/register.jsx`
- [x] T019 更新根布局，包裹 AuthProvider `apps/mobile/app/_layout.jsx`
...
- [x] T020 实现登录 API 端点（POST /api/auth/login，包含参数校验、bcrypt 验证、Session 创建） `apps/server/src/routes/auth.js`
- [x] T021 实现登出 API 端点（POST /api/auth/logout，删除 Session） `apps/server/src/routes/auth.js`
- [x] T022 实现获取当前用户信息 API 端点（GET /api/auth/me，需 requireAuth） `apps/server/src/routes/auth.js`
- [x] T023 在 use-auth Hook 中新增 login、logout、fetchCurrentUser 方法 `apps/mobile/hooks/use-auth.js`
- [x] T024 实现登录页面（表单、前端校验、提交逻辑、跳转到主页面） `apps/mobile/app/(auth)/login.jsx`
...
- [x] T025 在注册页面添加"返回登录"链接，点击跳转到 /login `apps/mobile/app/(auth)/register.jsx`
- [x] T026 在登录页面添加"立即注册"链接，点击跳转到 /register `apps/mobile/app/(auth)/login.jsx`
...
- [x] T027 在注册页面前端校验密码长度（最少 6 位，最多 128 位），显示友好提示 `apps/mobile/app/(auth)/register.jsx`
- [x] T028 在登录和注册页面设置密码输入框为 secureTextEntry 模式（掩码显示） `apps/mobile/app/(auth)/login.jsx` `apps/mobile/app/(auth)/register.jsx`
