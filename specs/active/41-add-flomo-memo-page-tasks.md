...
- [ ] T120 创建或更新 `.github/workflows/ci.yml`，配置 CI 流程（install → lint → build → test:unit → test）
- [ ] T121 确保 CI 流程中包含数据库迁移步骤（db-setup → db-migrate）
- [ ] T122 确保 CI 流程中包含后端和前端服务启动步骤（fullstack-start）
...
- [ ] T137 将所有改动提交到 `feat/41-flomo-memo-page` 分支
- [ ] T138 创建 Pull Request，关联 Issue #41
- [ ] T139 在 PR 描述中列出所有实现的功能和测试覆盖情况
- [ ] T140 等待 CI 通过后请求 Code Review
