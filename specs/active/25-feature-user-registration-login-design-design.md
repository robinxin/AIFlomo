...
**创建日期**: 2026-03-04
**状态**: 待审核
**技术栈**: Node.js + Fastify + Drizzle ORM + SQLite + Expo (React Native) + JavaScript

---

## 1. 功能概述

本次功能实现 AIFlomo 应用的用户账号注册与登录流程，构建身份认证基础设施。

**核心目标**：
- 用户通过邮箱 + 密码完成注册，注册成功后自动登录并进入笔记列表页
- 用户通过邮箱 + 密码登录，验证成功后创建 Session 并跳转到笔记列表页
- 登录状态通过 Cookie-based Session 持久化，默认 7 天有效期

**系统定位**：
- **后端**：新增 `/api/auth/register`、`/api/auth/login`、`/api/auth/logout`、`/api/auth/me` 四个端点（前两个为公开端点，后两个需认证）
- **数据库**：新增 `users` 表存储用户基本信息，`sessions` 表由 Fastify session 插件自动管理
- **前端**：新增 `(auth)` 路由组，包含 `/login` 和 `/register` 两个页面，与主应用路由隔离
- **全局状态**：新增 `AuthContext` 管理用户登录状态（`user` 对象、`isAuthenticated` 布尔值）

**用户价值**：
- 解决用户如何安全访问个人笔记数据的问题
- 支持用户关闭应用后再次打开时自动登录（Session 未过期情况下）
- 为后续笔记 CRUD、标签管理等功能提供用户身份上下文（`userId`）
...
**字段说明**：
- `id`：用户唯一标识（UUID 主键），由 `crypto.randomUUID()` 自动生成
- `email`：用户邮箱，**唯一索引**，注册和登录时统一转为小写存储（防止大小写重复）
- `passwordHash`：bcrypt 哈希后的密码（salt rounds = 10），**禁止明文存储**
- `nickname`：用户昵称（1-20 字符），用于前端展示
- `createdAt`：注册时间，SQLite `CURRENT_TIMESTAMP` 自动生成（ISO 8601 格式）
- `lastLoginAt`：最后登录时间，每次登录成功时更新（初始为 `null`）

**索引设计**：
- `email` 字段通过 `.unique()` 修饰符自动创建唯一索引
- MVP 阶段用户量小，无需额外复合索引
...
使用 `@fastify/session` + `@mgcrea/fastify-session-sqlite-store` 插件，Session 数据存储在 SQLite 同库，表结构如下（**无需手动在 schema.js 中定义**）：
...
| 409 | `EMAIL_EXISTS` | "该邮箱已注册，请直接登录" | 邮箱已存在（捕获 SQLite UNIQUE 约束错误） |
...
**实现要点**:
- 捕获 SQLite `SQLITE_CONSTRAINT` 错误（邮箱唯一约束冲突），返回 409 状态码
...
**安全要点**:
- 邮箱不存在和密码错误统一返回"邮箱或密码错误"，防止攻击者枚举已注册邮箱
...
const styles = StyleSheet.create({
  ...
  link: { marginTop: 16, textAlign: 'center', color: '#4caf50' },
});
```
...
- 昵称输入框
- 密码输入框
- 隐私协议勾选框（使用 `Pressable` 实现自定义 Checkbox）
- 注册按钮（未勾选协议时禁用）
- "返回登录" 链接（`<Link href="/login">`）

**状态管理**:
- 组件内部 `useState` 管理表单字段（`email`, `nickname`, `password`, `agreePrivacy`, `error`）
- 注册成功后，自动登录（后端已设置 Session），直接 `router.replace('/memo')`

**错误处理**:
- **前端校验**：
  - 邮箱格式（正则 `/^[^\s@]+@[^\s@]+\.[^\s@]+$/`）
  - 密码 8-20 位且包含字母和数字（正则 `/^(?=.*[A-Za-z])(?=.*\d).{8,20}$/`）
  - 昵称 1-20 字符
  - 协议必须勾选
...
function RootLayoutInner() {
  ...
  if (!state.isAuthenticated) return <Redirect href="/login" />;
...
│   └── migrations/                    # Drizzle 自动生成的迁移文件（执行 npm run db:generate 后生成）
├── routes/
│   └── auth.js                        # 新增认证路由（register, login, logout, me）
├── plugins/
│   ├── session.js                     # 新增 Fastify session 插件配置
│   └── auth.js                        # 新增 requireAuth preHandler
├── lib/
│   └── password.js                    # 新增密码工具函数（bcrypt 封装）
└── index.js                           # 新增应用入口（Fastify 实例 + 全局错误处理）
```
...
│   │   ├── login.jsx                  # 登录页
│   │   └── register.jsx               # 注册页
│   └── _layout.jsx                    # 新增根布局（集成 AuthProvider + useCheckAuth）
├── context/
│   └── AuthContext.jsx                # 新增 AuthContext
├── hooks/
│   └── use-check-auth.js              # 新增登录状态检测 Hook
└── lib/
    └── api-client.js                  # 新增 API Client（统一 fetch 封装）
```
...
```bash
# 根目录 .env（需提交 Git）
SESSION_SECRET=<32-char-random-string>  # 新增 Session 密钥，使用 crypto.randomBytes(32).toString('hex') 生成
EXPO_PUBLIC_API_URL=http://localhost:3000  # 前端 API 地址
NODE_ENV=development                    # 开发环境
PORT=3000                               # 后端端口
DB_PATH=./data/aiflomo.db               # SQLite 数据库路径
CORS_ORIGIN=http://localhost:8081       # CORS 白名单
```

### 5.3 依赖包新增

#### 后端新增依赖（apps/server/package.json）

| 包名 | 版本 | 用途 |
|------|------|------|
| `fastify` | `^5.0.0` | Web 框架 |
| `drizzle-orm` | `^0.36.0` | ORM |
| `drizzle-kit` | `^0.29.0` | Drizzle 迁移工具 |
...
| `@fastify/cookie` | `^10.0.0` | Cookie 解析 |
| `@fastify/cors` | `^10.0.0` | CORS 支持 |
| `fastify-plugin` | `^5.0.0` | Fastify 插件工具 |

#### 前端新增依赖（apps/mobile/package.json）
...
| `react-native` | `^0.76.0` | React Native 核心 |
...
| `password` | string | 8-20 字符 | 必须包含字母和数字（正则：`/^(?=.*[A-Za-z])(?=.*\d).{8,20}$/`） | "密码需为 8-20 位，包含字母和数字" |
...
10. **Session 跨域共享**：不支持多个域名共享 Session（仅限单一域名部署）
