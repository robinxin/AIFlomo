...
│       ├── deploy-server.sh   # 构建并部署后端（pm2）
│       └── deploy-frontend.sh # 构建并部署前端（pm2）
...
| `deploy-server.sh` | 构建并部署后端（pm2） | `npm run build -w apps/server` + pm2 |
| `deploy-frontend.sh` | 构建并部署前端（pm2） | `EXPO_USE_METRO_WORKSPACE_ROOT=1 npm run build -w apps/mobile` + pm2 |
