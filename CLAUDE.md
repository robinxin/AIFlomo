...
### 容错处理规范
- 所有调用 workspace 的 CI 脚本（`db-*.sh`、`server-*.sh` 等）必须容错处理 workspace 不存在的情况
- 使用 `2>/dev/null || echo "Skipped: ..."` 模式，确保脚本不因 workspace 缺失而失败
- 示例：`npm run db:migrate -w apps/server 2>/dev/null || echo "Skipped: apps/server workspace not found"`
