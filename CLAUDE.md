...
npm run dev                           # 启动后端（当前项目仅包含 apps/server）
...
npm run prod           # pm2 start src/index.js --name aiflomo-server
npm run db:generate    # 生成 Drizzle client
npm run db:migrate     # 执行数据库迁移
npm run db:reset       # 重置数据库（删除所有数据并重新运行迁移）

# ── 前端（apps/mobile，Expo）──
# ⚠️  当前项目暂未实现前端子包，此章节待补充

# ── 单测 ──
npm run test:unit      # 运行后端单测（Jest）

cd apps/server
npm run test:unit      # 后端 Jest 单测
npm run test:unit:cov  # 后端单测 + 覆盖率报告

# ── E2E 测试 ──
npm test               # Playwright E2E 测试
npm run test:ui        # Playwright E2E 测试 UI 模式
...
│   │   ├── package.json   # 子包配置（必须包含 dev/build/lint/prod 脚本）
│   │   └── drizzle.config.js
│   └── tests/             # Playwright E2E 测试（*.spec.js）
│
│   # ⚠️  apps/mobile/ (前端子包) 当前暂未实现，待补充
├── testcases/             # 测试用例描述文件
├── docs/                  # 详细技术文档
...
- **静态文件**: 前端产物由 Nginx 托管（待实现）
- **环境变量**: 通过 `.env.production` 注入，不得提交到 Git

```bash
# 典型上线流程（当前仅后端）
npm run build -w apps/server      # 编译后端（当前为空操作）
npm run prod  -w apps/server      # pm2 启动 / 重载后端服务
# 前端部署流程待补充
```
